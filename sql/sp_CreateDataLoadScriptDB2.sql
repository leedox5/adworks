IF EXISTS (
	...
	WHERE SPECIFIC_NAME = 'sp_CreateDataLoadScriptDB2'
)
	DROP PROCEDURE sp_CreateDataLoadScriptDB2
GO

CREATE Procedure [dbo].[sp_CreateDataLoadScriptDB2]
------------------------------------------------------------------------------------------
-- NAME
...
as begin
	...
	while len(@TblName)>0 begin
		...
		select	@cmd1 = 'insert into #out select ' + cast(@lnr as varchar) + ', '' insert into RMSFLE.' + @TblName   + '  '
		select	@cmd2 = ' + '' select '' + '
		while @id < @maxid
		begin
			...
			select @cmd2 = 	@cmd2
					...
					+	' else '
					+	  case 
                             when ColType = 1 then  ' left('''''''' + replace(convert(varchar(max),[' + ColName + ']),'''''''','''''''''''') + ''''''''+''                                     '',20)' 
                             else 'left(convert(varchar(50),[' + ColName + '])+''                                 '',20) ' 
                          end
					+ ' end + '','' + '
			from	#a
			where	id = @id
        ...
        select @cmd1 = left(@cmd1,len(@cmd1)) + '  '' '
	...
	while @@fetch_status=0 begin
		...
		print @statement + ' from sysibm.sysdummy1 ;'
